...
def doctor_ai_suggestions():
    ...
    cur = db.cursor()
    cur.execute("""
        SELECT s.id, u.name, s.suggestion_text, s.verified, s.timestamp, s.doctor_feedback, s.doctor_status
        FROM ai_suggestions s
        JOIN users u ON s.patient_id = u.id
        WHERE u.assigned_doctor_id = %s
        ORDER BY s.timestamp DESC
    ...
    suggestions_list = cur.fetchall()
    db.close()
    return render_template('doctor_ai_suggestions.html', suggestions_list=suggestions_list)
...
def doctor_profile():
    ...
    if request.method == 'POST':
        name = request.form['name']
        email = request.form['email']
        # Specialization and contact info are not in the current schema,
        # so we'll just update name and email for now.
        # Password update would require a separate form field and hashing.
